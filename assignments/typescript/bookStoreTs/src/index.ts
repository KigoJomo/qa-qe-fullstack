  ...
  books.forEach((book) => {
    ...
    bookCard.innerHTML = `
      <div
        ...
        <div
          class="details w-full aspect-[3/4] col-span-1 row-span-1 z-[3] flex flex-col justify-end gap-2 p-4">
          <h4 class="text-white font-bold">${book.title}</h4>
          <hr class="border-white" />
          <div class="w-full flex items-center gap-4">
            <p class="text-white capitalize">${book.year}</p>
            <p class="text-white">-</p>
            <p class="text-white capitalize">${book.author}</p>
          </div>
          <p class="text-gray-700 capitalize bg-blue-100 w-fit border-blue-900 px-4 py-1 rounded-full">${
            book.genre
          }</p>
